...
(defun create-test-match-tree ()
  (let ((root (http-routes.matcher::make-node)))
    ...
    (http-routes.matcher::add-to-tree root (http-routes.matcher::create-pattern '("control/" :wildcard)) :control-wildcard)
    (http-routes.matcher::add-to-tree root "control/login" :control-login)
    (http-routes.matcher::add-to-tree root "orders" :orders)
    (http-routes.matcher::add-to-tree root (http-routes.matcher::create-pattern '("orders/" :wildcard)) :order-id)
    (http-routes.matcher::add-to-tree root (http-routes.matcher::create-pattern '("orders/" :wildcard "/history")) :order-id-history)
    (http-routes.matcher::add-to-tree root (http-routes.matcher::create-pattern '("orders/" :wildcard "/history/" :wildcard)) :order-id-history-item-id)
    (http-routes.matcher::add-to-tree root (http-routes.matcher::create-pattern '("orders/" :wildcard "/history/" :wildcard "/name")) :order-id-history-item-id-name)
...
(test (matcher.1 :compile-at :definition-time)
  (let ((root (create-test-match-tree)))
    ...
    (multiple-value-bind (matched tag index) (http-routes.matcher::match root "orders/123/history/456/name")
      ...
      (is-true (equalp index '((7 10) (19 22)))))
